...
const SHARED_SELECTORS: Record<string, string> = {
  ...
  '.drag-handle': 'block handle rendered beside the content root; scoping candidate',
...
function flatten(css: string, from: string): Root {
  return postcss([nested({ bubble: ['media', 'supports', 'layer', 'container'] })]).process(
    css,
    { from },
  ).root;
}
...
function collectRules(container: Container, out: Rule[]): void {
  container.each((node) => {
    ...
    else if (node.type === 'atrule' && !/keyframes$|^font-face$/.test(node.name)) {
...
function isPositive(node: SelectorNode): boolean {
  ...
  return node.type === 'class' && OWNED_PREFIXES.some((p) => node.value.startsWith(p));
}
...
function isOwned(selector: Selector): boolean {
  ...
  return compounds.some(
    (c, i) =>
      isAnchor(c) &&
      (i === compounds.length - 1 || combinators[i] === ' ' || combinators[i] === '>'),
  );
}
...
describe('css ownership', () => {
  it('finds the first-party stylesheets', () => {
    expect(cssFiles.map(rel)).toEqual(
      expect.arrayContaining(['styles/index.css', 'styles/editor.css']),
    );
  });
...
describe('ownership predicate', () => {
  ...
  it.each([
    ...
    ['.ProseMirror { :is(&, body) {} }', [':is(.ProseMirror, body) (fixture.css:1)']],
